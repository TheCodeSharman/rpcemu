...
On an unfixed driver the 8MB and 32MB cases error out in the fixture: the abort
stops the boot before the desktop, so the guest never becomes usable. (HostCmd
itself still answers -- it comes up at the supervisor prompt, well before the
abort -- which is why readiness is judged by Wimp$State, not by a reply.) The
256MB cases pass either way: they are the control, proving the fix does not
regress the one size that always worked. Verified in both directions against
the fixed and unfixed drivers.
...
# s/errors defines ERR_EINVAL as 0x20E16, but the driver returns a pointer past
# the block, so clients never see that number. test_swi_rejects_bad_flags checks
...
def _booted(install: Path, mem_size: int):
    ...
    try:
        ...
        # HostCmd answers from the supervisor prompt ~4s in, seconds before the
        # desktop exists -- a command run in that window has no application slot
        # and data-aborts. Wimp$State reaching "desktop" means the boot finished,
        # which an unfixed driver's abort never lets it do.
        deadline = time.monotonic() + BOOT_TIMEOUT
        while time.monotonic() < deadline:
            try:
                if "desktop" in guest.run("Show Wimp$State", timeout=15)[1]:
                    break
            except subprocess.TimeoutExpired:
                pass
            time.sleep(2)
        else:
            pytest.fail(f"{mem_size}MB: guest never reached the desktop "
...
def test_swi_rejects_bad_flags(guest_at_ram):
    ...
    Only the fact of an error is asserted, not its number: the driver returns a
    pointer past its own ERR_EINVAL block, so the number a client reads is
    whatever happens to sit there. The unfixed driver does the same, so this is
    pre-existing and out of scope here.
    """
    guest, mem_size = guest_at_ram
    out = _basic(guest, f'SYS &{SWI_DCIVERSION:X},1 TO ,v%\nBPUT#o%,"no error"')
    assert out.startswith("ERR &"), f"{mem_size}MB: expected an error, got {out!r}"
